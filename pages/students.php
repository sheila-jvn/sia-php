...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
    <form action="" method="GET" class="flex flex-1 gap-2">
        <input type="text" name="search" class="flex-1 rounded-lg border border-secondary-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-400 bg-white text-sm" placeholder="Cari data siswa..." value="<?= htmlspecialchars($searchQuery) ?>">
        <button class="inline-flex items-center gap-1 px-4 py-2 rounded-lg bg-primary-600 text-white hover:bg-primary-700 transition" type="submit">
            <iconify-icon icon="cil:search" class="text-lg"></iconify-icon> Cari
        </button>
        <?php if ($searchQuery): ?>
            <a href="students" class="inline-flex items-center gap-1 px-4 py-2 rounded-lg border border-secondary-300 text-secondary-700 bg-white hover:bg-secondary-100 transition">Reset</a>
        <?php endif; ?>
    </form>
    <div class="flex gap-2">
        <a href="<?= htmlspecialchars($urlPrefix) ?>/students/create" class="inline-flex items-center gap-1 px-4 py-2 rounded-lg bg-primary-600 text-white hover:bg-primary-700 transition">
            <iconify-icon icon="cil:plus" class="text-lg"></iconify-icon> Tambah Data
        </a>
        <a href="students/export<?= $searchQuery ? ('?search=' . urlencode($searchQuery)) : '' ?>" class="inline-flex items-center gap-1 px-4 py-2 rounded-lg bg-accent-500 text-white hover:bg-accent-600 transition">
            <iconify-icon icon="cil:file-export" class="text-lg"></iconify-icon> Export Data
        </a>
    </div>
</div>
                ...
                <?php foreach ($students as $siswa): ?>
                    <tr class="even:bg-secondary-50 hover:bg-secondary-100">
                        ...
                        <td class="px-4 py-2 whitespace-nowrap flex gap-1">
                            <a href="students/details?id=<?= htmlspecialchars($siswa['id']) ?>" class="inline-flex items-center justify-center p-2 rounded-lg bg-status-info-100 text-status-info-700 hover:bg-status-info-200 transition" title="Detail">
                                <iconify-icon icon="mdi:eye-outline"></iconify-icon>
                            </a>
                            <a href="students/edit?id=<?= htmlspecialchars($siswa['id']) ?>" class="inline-flex items-center justify-center p-2 rounded-lg bg-status-warning-100 text-status-warning-700 hover:bg-status-warning-200 transition" title="Edit">
                                <iconify-icon icon="mdi:pencil-outline"></iconify-icon>
                            </a>
                            <a href="students/delete?id=<?= htmlspecialchars($siswa['id']) ?>" class="inline-flex items-center justify-center p-2 rounded-lg bg-status-error-500 text-white hover:bg-status-error-600 transition" title="Hapus">
